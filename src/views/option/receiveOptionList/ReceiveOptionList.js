...
const renderEmpty = () => {
    return (
        <View style={styles.listEmptyContainer}>
            <Text style={[globalStyles.largeText, styles.listEmptyText]}>暂无经销商</Text>
        </View>
    )
}

const ReceiveOptionList = props => {
    const { receiveOptionListReducer: { data: { receiveOptionList }, getReceiveOptionList }, onSelect, selectedItem, cleanSelected, optionalSearchFormValues } = props
    let list = receiveOptionList
    if (selectedItem) {
    ...
    if (getReceiveOptionList.isResultStatus == 1) {
        return (
            <Container>
...
const mapStateToProps = (state) => {
    return {
        receiveOptionListReducer: state.receiveOptionListReducer,
        optionalSearchFormValues: getFormValues('optionalSearchForm')(state)
    }
}


export default connect(mapStateToProps)(ReceiveOptionList)